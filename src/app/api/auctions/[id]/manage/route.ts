...
export async function PUT(
  ...
  try {
    ...
    // Check if auction has started (current time > start time)
    const now = new Date()
...
export async function DELETE(
  ...
  try {
    ...
    // Get all bids with payment authorization info
    const { data: bids, error: bidsError } = await supabaseAdmin
      .from('bids')
      .select('*')
      .eq('auction_id', auctionId)
    ...
    // If there are bids, we need to cancel their payment authorizations
    if (bids && bids.length > 0) {
      console.log(`Found ${bids.length} bids to cancel for auction ${auctionId}`)
      
      // Cancel all payment authorizations
      for (const bid of bids) {
        if (bid.stripe_payment_intent_id && bid.authorization_status === 'authorized') {
          try {
            if (bid.stripe_payment_intent_id.startsWith('pi_test_')) {
              // Mock payment intent - just log
              console.log('Cancelling mock payment authorization:', bid.stripe_payment_intent_id)
            } else {
              // Real Stripe payment intent
              await StripeService.cancelPaymentAuthorization(bid.stripe_payment_intent_id)
              console.log('Cancelled payment authorization:', bid.stripe_payment_intent_id)
            }
          } catch (error) {
            console.error('Failed to cancel payment authorization:', error)
            // Continue with deletion even if some authorizations fail to cancel
          }
        }
      }

      // Update bid statuses to cancelled
      await supabaseAdmin
        .from('bids')
        .update({ 
          status: 'cancelled',
    ...
    // Delete auction images first (if any)
    ...
    if (imagesError) {
      ...
      // Continue with auction deletion even if images fail to delete
    }

    // Delete the auction (this will cascade delete bids due to foreign key)
    ...
    return NextResponse.json({
      success: true,
      message: `Auction deleted successfully. ${bids ? bids.length : 0} bid authorizations cancelled.`
    })
