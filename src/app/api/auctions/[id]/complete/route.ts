...
export async function POST(
  ...
  try {
    ...
    console.log('ðŸ STARTING AUCTION COMPLETION FOR:', auctionId)
    ...
    if (bidError || !winningBid) {
      ...
      console.log('âœ… Auction ended with no bids')
      return NextResponse.json({
        success: true,
    ...
    // Capture the authorized payment to BidWin account first
    if (winningBid.stripe_payment_intent_id) {
      try {
        ...
        if (paymentIntent.status === 'requires_capture') {
          // Step 1: Capture full amount to BidWin account
        ...
        if (capturedPayment.status === 'succeeded') {
          console.log(`ðŸ’° Captured ${winningBid.amount} to BidWin account for auction ${auctionId}`)
          ...
          if (auctionUpdateError) {
            console.error('âŒ WARNING: Failed to update auction status:', auctionUpdateError)
            ...
            try {
              ...
              if (fallbackError) {
                ...
                console.log('âœ… FALLBACK: Auction status updated')
              }
            } catch (e) {
              console.error('Fallback attempt error:', e)
            }
          } else {
            console.log('âœ… AUCTION STATUS UPDATED TO COMPLETED')
          }
          ...
          if (sellerProfile?.stripe_account_id) {
            try {
              ...
              console.log(`ðŸ’¸ Transferred ${sellerAmount} to seller account ${sellerProfile.stripe_account_id}`)
              console.log(`ðŸ¢ BidWin keeps platform fee: ${platformFee}`)
            } catch (transferError) {
              console.error('Transfer to seller failed:', transferError)
              // Continue anyway - we can retry transfer later
              // Funds are safely in BidWin account
            }
          }
          ...
          if (bidUpdateError) {
            ...
            console.log('âœ… Winning bid status updated to captured/winning')
          }
          ...
          if (otherBids) {
            ...
            if (outbidUpdateError) {
              ...
              console.log('âœ… Outbid statuses updated')
            }
          }

          console.log('ðŸŽ‰ AUCTION COMPLETION SUCCESSFUL!');
          ...
          return NextResponse.json({
            success: true,
            message: `Auction completed! $${winningBid.amount} captured to BidWin. $${sellerAmount} transferred to seller, $${platformFee} platform fee retained.`,
            winner: winningBid.users,
