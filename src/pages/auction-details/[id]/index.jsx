...
const AuctionDetails = () => {
  ...
  const handlePlaceBid = async (amount, paymentMethodId) => {
    if (!auction?.id) return Promise.reject('No auction available');
    if (!user) return Promise.reject('Authentication required');
    if (!paymentMethodId) return Promise.reject('Payment method is required');
    ...
    try {
      ...
      const { new_current_price } = await response.json();
      ...
      <main className="container mx-auto px-4 py-6 max-w-7xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            {/* Image Gallery */}
            <ImageGallery 
              images={auction.images?.map(img => img.url || img.image_url) || [auction.primary_image].filter(Boolean)} 
              title={auction.title} 
            />
                      ...
                      {auction.shipping_methods?.map((method, index) => (
                        <div key={index} className="flex justify-between">
                          ...
                          <span className="font-medium">
                            {auction.shipping_cost > 0 ? `S$${auction.shipping_cost.toFixed(2)}` : 'Free'}
                          </span>
                        </div>
                      )) || (
