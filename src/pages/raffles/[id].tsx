...
import { loadStripe, Stripe, StripeElements, PaymentElement, StripePaymentElement } from '@stripe/stripe-js'
          ...
          <div className="bg-card border border-border rounded-lg p-4">
            ...
            <div className="space-y-3 text-sm text-foreground">
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold">${(raffle.ticket_price / 100).toFixed(2)} SGD</span>
              </div>
              <div>
            ...
            {raffle.status !== 'active' || remaining === 0 ? (
              ...
              <>
                ...
                <div className="flex items-center justify-between mb-3 text-sm">
                  <span>Total</span>
                  <span className="font-semibold">${((raffle.ticket_price * quantity) / 100).toFixed(2)} SGD</span>
                </div>
                <Button fullWidth onClick={beginPurchase} iconName="CreditCard" disabled={purchaseInProgress || !!clientSecret}>
                  {purchaseInProgress ? 'Preparing…' : 'Start Payment'}
                </Button>
                <p className="text-xs text-muted-foreground mt-2">You’ll complete payment securely with Stripe.</p>
              </>
            )}
          </div>
        </div>
