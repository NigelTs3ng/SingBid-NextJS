...
const AuctionCompletionPanel = ({ auction, currentUser, onAuctionCompleted }) => {
  ...
  const handleCompleteConfirm = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        alert(result.message || 'Auction completed successfully!');
      }, 100);

    } catch (error) {
      console.error('Completion error:', error);
      alert(`Failed to complete auction: ${error.message}`);
