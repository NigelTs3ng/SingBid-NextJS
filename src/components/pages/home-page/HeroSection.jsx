...
const HeroSection = () => {
  ...
  const fetchFeaturedAuctions = async () => {
    try {
      // Fetch top 3 auctions by current price that are active
      ...
      const transformedAuctions = auctionsData?.map(auction => {
        ...
        return {
          ...
          image: auction.primary_image || auction.images?.[0]?.image_url || 'https://images.unsplash.com/photo-1517336714731-489689fd1ca8?w=800&h=600&fit=crop',
          seller: {
            name: auction.seller?.name || auction.seller?.email || 'Anonymous Seller',
            rating: 4.5, // Default rating since we don't have ratings in our schema yet
            verified: true
          },
