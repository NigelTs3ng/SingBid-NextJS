...
export class StripeService {
  ...
  // Create payment intent for authorization only (hold funds in SingBid account)
  static async authorizePayment(
    ...
    return await stripe.paymentIntents.create({
      amount: Math.round(amount * 100), // Convert to cents - FULL AMOUNT TO SINGBID
      currency,
      customer: customerId,
      // NO application_fee_amount or transfer_data - funds stay in SingBid account
